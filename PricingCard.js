import { IconCheck, IconSparkle } from "./PricingIcons";
 ...
 *
 * Visual-only redesign: props, CTA rendering, and default button
 * markup are unchanged, so existing gating/logic that renders a
 * custom `cta` keeps working exactly as before.
 *
 * @param {object} props
...
export default function PricingCard({
  ...
  return (
    <div
      className={`card ${styles.card} ${highlighted ? `${styles.highlighted} glass` : ""}`.trim()}
    >
      {highlighted && <span className={styles.glow} aria-hidden="true" />}

      {highlighted && (
        <span className={`badge badge-accent ${styles.ribbon}`}>
          <IconSparkle className={styles.ribbonIcon} aria-hidden="true" />
          Most popular
        </span>
      )}
      ...
      <ul className={styles.features}>
        {features.map((feature) => (
          <li key={feature}>
            <span className={`${styles.check} ${highlighted ? styles.checkAccent : ""}`} aria-hidden="true">
      ...
      {cta ?? (
        <button
          type="button"
          className={`btn ${highlighted ? "btn-primary" : "btn-secondary"} ${styles.cta}`}
        >
          {ctaLabel}
        </button>
      )}
