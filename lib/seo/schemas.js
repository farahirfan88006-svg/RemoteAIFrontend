...
/**
 * FAQPage schema for a blog post that includes an FAQ section — enables
 * FAQ rich results in Google search. `faqs` is the same
 * [{ question, answer }] array stored on the post (see lib/blog/posts.js);
 * answers must be plain text, matching what's rendered on the page.
...
export function buildArticleSchema(post = {}) {
  return {
    ...
    // Falls back to publishedAt when a post has no distinct updatedAt —
    // never invents a revision date that didn't happen.
    dateModified: post.updatedAt || post.publishedAt,
    image: [post.featuredImage?.url || `${siteConfig.url}${siteConfig.socialImage}`],
    author: {
      "@type": "Person",
