...
function toSummary(post) {
  return {
    ...
    // `updatedAt` is optional on the source post — only set it when a post
    // has genuinely been revised. Consumers should treat `updatedAt` as
    // "same as published" whenever it's absent, rather than assume a
    // separate revision happened.
...
/**
 * Previous/next article navigation, ordered chronologically by
 * publishedAt (ascending) rather than by category — a simple, predictable
 * "earlier article / later article" reading order that works the same
 * way regardless of which category or tag the current post belongs to.
 * @param {{ slug: string }} post
 * @returns {{ previous: object|null, next: object|null }}
 */
export function getAdjacentPosts(post) {
  const ordered = [...BLOG_POSTS].sort((a, b) => new Date(a.publishedAt) - new Date(b.publishedAt));
  const index = ordered.findIndex((p) => p.slug === post.slug);
  if (index === -1) return { previous: null, next: null };
  return {
    previous: index > 0 ? toSummary(ordered[index - 1]) : null,
    next: index < ordered.length - 1 ? toSummary(ordered[index + 1]) : null,
  };
}
